...
- [x] **Backend module split — done 2026-09-24, deployed to prod 2026-09-25 (`c6e5d88`, includes both flaky-test bug fixes).** Verified on prod with a live smoke test covering every module: login/logout, getMyInfo/getUsers, fetchFollowedPosts, getMyFollows, getUnseenNotificationCount, and a full media upload→fetch→delete round-trip — all passed. No DB schema change involved (pure code move), so no migration step needed this time. All 7 modules deployed to dev first and verified there: Ran via looped `/split-backend-modules` invocations, one module per iteration (move → verify locally → commit → push → deploy to dev → run test suite): auth (`0868f0b`), users (`543f515`), posts (`c6e4e9f`), comments (`be056e8`), follows (`f16bb33`), notifications (`5c93b9b`), media (`a3df6b1`). `api.php`/`media.php` are now thin — only `handle_deleteAccount` still lives directly in `api.php` (deliberately, cascades across every domain). Real bugs the move itself caught, not just reorganized code:
...
## Done (recent)
- **TUI registration flow tested end-to-end against both dev and prod (2026-09-25), prompted by a friend reporting no OTP email on registration.** Pulled and rebuilt `simple-social-tui` to the latest commit first (was 8 commits behind — only visual/rendering changes in that gap, register logic itself untouched). Drove the real Register screen in a tmux session against dev, then again against prod: email → OTP prompt → password → confirm → auto-login to feed, worked cleanly both times, no TUI bug. Checked the actual mail server logs for both test sends: postfix DKIM-signed each one (`d=app.davidfruin.com`) and Gmail's own server accepted both with a clean `250 2.0.0 OK` — the sending pipeline itself is healthy on both dev and prod right now, not a systemic failure. The friend's case is more likely spam-filing (Gmail can still junk a technically-accepted message) or a stricter provider than Gmail (Outlook/Yahoo are more aggressive with newer-domain mail) — worth asking him which provider he used and to check spam. Test accounts cleaned up on both dev and prod afterward; this machine's shared CLI config was pointed at prod for the prod test and restored back to dev immediately after.
